...
public class Relatorio_OfertaDisciplinaController extends HttpServlet {
    ...
    switch (opcao) {
        case "Buscar":
            ...
            Model.Curso umCurso = new Model.Curso();
            umCurso.setNome(request.getParameter("curso"));
            umCurso.pesquisarCodCurso();
            Model.Disciplina disciplina = new Model.Disciplina();
            disciplina.pesquisarTodos(umCurso.getCodigo());
            umCurso.pesquisar();
            request.setAttribute("d", disciplina);
            request.setAttribute("c", umCurso);
            RequestDispatcher dispatcher = request.getRequestDispatcher("relatorio_oferta_materias_preenchida.jsp");
            dispatcher.forward(request, response);
            break;
      case "preencher":
            Model.Resultados resultados = new Model.Resultados();
            request.setAttribute("results", resultados);
            RequestDispatcher dispatcher2 = request.getRequestDispatcher("relatorio_oferta_materias.jsp");
